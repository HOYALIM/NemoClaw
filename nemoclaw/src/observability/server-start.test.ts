...
function makeFakeServer({
  ...
  const server: FakeServer = {
    once: vi.fn((event: string, handler: ErrorHandler) => {
      event === "error" && (server.errorHandler = handler);
      return server;
    }),
    off: vi.fn(() => server),
    listen: vi.fn((_port: number, _host: string, callback: ListenCallback) => {
      listenError ? server.errorHandler?.(listenError) : callback();
      return server;
    }),
...
describe("startMetricsServer", () => {
  ...
  it("starts and closes the metrics server", async () => {
    ...
    expect(state.server?.closeIdleConnections).toHaveBeenCalled();
    expect(state.server?.closeAllConnections).toHaveBeenCalled();
  });
