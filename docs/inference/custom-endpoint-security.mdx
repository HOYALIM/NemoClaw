...
Custom endpoint onboarding has one narrower operator-controlled exception for corporate inference
gateways.
Set `NEMOCLAW_TRUSTED_PRIVATE_INFERENCE_HOSTS` to a comma-separated list of exact hostnames or IP
literals to admit an endpoint on RFC1918, CGNAT, or IPv6 ULA space.
NemoClaw still resolves DNS, pins the validation connection, and rejects wildcard or suffix matches,
link-local metadata, reserved destinations, and resolver failures.
