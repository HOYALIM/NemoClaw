...
For an active messaging channel, onboarding validates a replacement credential before it backs up, recreates, and restores the sandbox.
...
| Active messaging credential | Rerun onboarding with the replacement value | Onboarding validates the credential before automatically backing up, recreating, and restoring the sandbox |
...
NemoClaw compares the replacement with the recorded hash for each active messaging credential, validates changed values before mutation, then automatically backs up, recreates, and restores the sandbox.
