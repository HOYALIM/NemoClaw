...
For an active messaging channel, onboarding validates a replacement credential before it backs up supported workspace and manifest-declared state, recreates the sandbox, and restores the backup.
...
| Active messaging credential | Rerun onboarding with the replacement value | Onboarding validates the credential before automatically backing up supported workspace and manifest-declared state, recreating the sandbox, and restoring that backup; files outside those state paths are not preserved |
...
NemoClaw compares the replacement with the recorded hash for each active messaging credential and validates changed values before mutation.
...
Files outside those state paths are not preserved.
If validation fails, onboarding leaves the existing sandbox unchanged.
...
The same onboarding flow evaluates other active messaging credentials, including `WECHAT_BOT_TOKEN` and `MSTEAMS_APP_PASSWORD`.
Supply the replacement credential together with any required channel configuration, or rerun onboarding interactively when the channel uses a guided credential flow such as WeChat pairing.
...
3. For messaging, use `channels remove` to retire the integration or rerun onboarding with the replacement credential so NemoClaw validates it before automatic recreation.
