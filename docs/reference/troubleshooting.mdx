...
Current NemoClaw stores SHA-256 hashes of messaging credentials in the sandbox registry at creation time and detects when a token for an active channel has changed.
It validates the replacement credential before it backs up workspace state, deletes the sandbox, recreates it with the new credential, and restores the backup.
A replacement token for a channel paused with `channels stop` does not trigger onboarding recreation because that channel is inactive.
