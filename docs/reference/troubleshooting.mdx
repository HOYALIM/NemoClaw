...
- A serving watchdog inside the sandbox stops a gateway process that no longer serves after returning at least one serving response, and the supervisor relaunches it.
...
After the gateway returns a serving response, these probe outcomes count as not serving:
...
The watchdog logs the cause of each not-serving probe only after a serving response.
It stops the gateway after four not-serving probes without an intervening serving response.
At the default 30-second interval, this takes roughly two minutes when there are no inconclusive probes.
...
An inconclusive probe leaves the gateway untouched and preserves the number of not-serving probes since the last serving response.
...
Rerunning `$$nemoclaw onboard --non-interactive` with a new `TELEGRAM_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, `SLACK_BOT_TOKEN`, or `SLACK_APP_TOKEN` previously reported success while the sandbox kept using the old credential.
When you rerun onboarding, NemoClaw compares each supplied token with its SHA-256 hash in the sandbox registry for active channels.
When you provide a replacement token, NemoClaw validates it before it backs up workspace state, destroys the sandbox, recreates it, and restores the backup.
If validation fails, onboarding stops before it backs up workspace state or destroys the existing sandbox.
If you do not supply a token, or the supplied token matches the recorded hash, the credential check does not trigger recreation.
If you replace a token for a channel that you stopped with `channels stop`, onboarding does not trigger recreation because that channel is inactive.
