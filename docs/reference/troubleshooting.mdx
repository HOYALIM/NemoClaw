...
Re-running `$$nemoclaw onboard --non-interactive` with a new `TELEGRAM_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, `SLACK_BOT_TOKEN`, or `SLACK_APP_TOKEN` previously reported success while the sandbox kept using the old credential.
...
When you provide a replacement token, NemoClaw validates it before backing up workspace state, deleting the sandbox, recreating it, and restoring the backup.
If validation fails, onboarding stops before it backs up or deletes the existing sandbox.
