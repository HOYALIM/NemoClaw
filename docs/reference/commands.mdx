...
After acquisition, it holds the lock until gateway recovery, forward repair, and any Hermes cron restore validation and gate release finish.
Hermes gate recovery can continue under that lock for its 130-second host timeout; the 30-second limit applies only to lock acquisition.
