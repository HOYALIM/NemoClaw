...
Remote overrides must use the official NVIDIA sandbox-base repository and resolve to an immutable repository digest. A repository-built local override must use its image-ID-derived content-addressed tag.
...
NemoClaw requires the official remote repository or an image-ID-bound local build, then validates the requested image against the manifest-required `deepagents-code` package version before using it.
...
NemoClaw requires the official remote repository or an image-ID-bound local build, validates the requested image for the required MCP runtime, and keeps the final image bound to that trusted base.
