...
The gate continues to block new Hermes gateway dispatch across gateway and container restarts in the same sandbox.
...
The command does not own or clear the Hermes operator drain, so dispatch remains blocked while that drain is active.
...
After NemoClaw creates the replacement, it acquires an independent root-owned gate that blocks new Hermes gateway dispatch.
