...
After NemoClaw creates the replacement, it blocks new Hermes gateway dispatch and waits for active agent work to finish before restoring state.
It validates the restored jobs and scripts against the same running gateway before it releases the drain.
If an operator already drained the gateway, NemoClaw preserves that drain instead of clearing it.
If state restore or cron validation fails, the command exits nonzero, preserves the backup, and leaves the Hermes gateway drained in the replacement sandbox.
</AgentOnly>
