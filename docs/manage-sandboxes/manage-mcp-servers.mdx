...
A discovery result is a point-in-time list from the configured MCP server, not an attestation of the exact tools visible to the model in an active agent session.
...
`recover` also owns this lock while it repairs the gateway and forwards and handles any Hermes cron restore gate.
