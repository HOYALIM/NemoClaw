...
const freshReonboardCheck = path.join(process.cwd(), DEEPAGENTS_FRESH_REONBOARD_CHECK);
...
describe("P0-E cloud-experimental parity guardrails", () => {
  it("skips the destructive fresh re-onboard check outside a Deep Agents sandbox", () => {
    ...
    try {
      ...
      const result = spawnSync("bash", [freshReonboardCheck], {
        encoding: "utf8",
        env: {
  ...
  it(
    "runs fresh re-onboard with a fake endpoint and no hosted inference secret",
    () => {
      ...
      try {
        ...
        writeExecutable(cli, [
          ...
          'if [ "${1:-}" = "onboard" ]; then',
          '  [ -z "${NVIDIA_INFERENCE_API_KEY:-}" ] || fail "NVIDIA_INFERENCE_API_KEY leaked"',
          '  [ "${NEMOCLAW_E2E_USE_HOSTED_INFERENCE:-}" != "1" ] || fail "hosted inference flag leaked"',
          '  [ "${COMPATIBLE_API_KEY:-}" = "e2e-compatible-key" ] || fail "missing fake compatible key"',
          '  [ "${NEMOCLAW_PROVIDER:-}" = "custom" ] || fail "missing custom provider"',
          '  [ "${NEMOCLAW_MODEL:-}" = "openai/openai/gpt-5.5" ] || fail "wrong target model"',
          '  case "${NEMOCLAW_ENDPOINT_URL:-}" in',
          "    http://127.0.0.1:*/v1) ;;",
          '    *) fail "unexpected endpoint ${NEMOCLAW_ENDPOINT_URL:-<unset>}" ;;',
          "  esac",
        ...
        const result = spawnSync("bash", [freshReonboardCheck], {
          ...
          env: {
            ...
            NVIDIA_INFERENCE_API_KEY: "sk-live-should-not-be-used",
            PATH: `${binDir}:${process.env.PATH ?? "/usr/bin:/bin"}`,
            REPO: process.cwd(),
        ...
        expect(result.stdout).toContain("04-deepagents-code-fresh-reonboard: 6 passed, 0 failed");
      } finally {
  ...
  it("does not pass the hosted inference key into the hermetic fresh re-onboard check", () => {
